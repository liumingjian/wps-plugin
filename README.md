...
Designated-Mac smoke check: with WPS (`wpsoffice`) installed, start the Demo, activate the Edit Entry, confirm a separate WPS Writer window opens `doc-001.docx`, make and persist a unique visible edit, then wait for the Demo page to report Editing Task `task-doc-001` at the `succeeded` stage. Download **Latest submitted result** and confirm the edit is present.
