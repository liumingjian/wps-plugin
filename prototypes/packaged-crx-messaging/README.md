...
It packages the production signing identity into two versions of one MV3 CRX,
...
versions `0.1.0` and `0.1.1`, installs the prototype Native Messaging host, and
...
2. Manually install `dist/wps-edit-packaged-prototype-0.1.1.crx` as an upgrade.
   Record whether the ID stays fixed, the version changes in place, and settings
